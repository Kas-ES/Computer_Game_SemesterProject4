...
public class WallHorizontal {

    private ArrayList<MapElementBlock> blockedPaths = new ArrayList<MapElementBlock>();
    private int boxSize = 10;

    public WallHorizontal(Room room, int length, float pX, float pY) {
        if (length <= 10) {
            this.boxSize = 1;
        } else {
        ...
        for (int val = 0; val <= length; val += this.boxSize) {
            this.blockedPaths.add(createMapElementSpecialB(pX + val, pY));
        }
    ...
    public WallHorizontal(Room room, int length, float pX, float pY, boolean entry) {
        if (length <= 10) {
            this.boxSize = 1;
        } else {
            this.boxSize = (int) length / 10;
